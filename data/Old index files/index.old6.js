...
// --- Consent / Touch Policy (top-level scope) ---
const CONSENT_PATH = path.resolve('./data/touch_consent.json');
let TOUCH_CONSENT = { users: {} }; // user-id -> true|false

function loadTouchConsent() {
  try {
...
function saveTouchConsent() {
  ...
  } catch(e) { logger.warn('Failed saving touch_consent.json: ' + (e?.message || e)); }
}
loadTouchConsent();
...
function isTrustedForContact(tags) {
  const name = (tags?.['display-name'] || tags?.username || '').toLowerCase();
  // Broadcaster always trusted; also allow explicit consent or trusted list
  return (typeof isBroadcasterUser === 'function' && isBroadcasterUser(tags)) ||
         TRUSTED_CONTACTS.has(name) ||
         getConsentForUser(tags) === true;
}

// Remove/soften physical-contact stage directions if not trusted
function applyConsentPolicy(str, tags) {
  if (!str) return str;
  if (isTrustedForContact(tags)) return str;
  // Strip common contact gestures
  const CONTACT_RX = /\*(?:hug|cuddle|nuzzle|rub|pet|snuggle|lean(?:s)? (?:on|against)|lick|kiss|smooch|caress|rubs? (?:against|on) [^*]*|nests? (?:into|against) [^*]*)\*/gi;
  let cleaned = str.replace(CONTACT_RX, '').replace(/\s{2,}/g, ' ').trim();
  // Soften general "*touches ...*" into a non-contact salute
  cleaned = cleaned.replace(/\*touch(?:es)?[^*]*\*/gi, '*offers a respectful salute*');
  return cleaned;
}

// Wrapper to send replies with consent applied
function sayWithConsent(channel, tags, msg, limit = 480) {
  const safe = applyConsentPolicy(msg, tags);
  return client.say(channel, twitchSafe(safe, limit));
}

// --- Touch Consent command (!tc yes|no) ---
async function handleTouchConsentCommand(channel, tags, args) {
  const uid = tags?.['user-id'];
  if (!uid) return;
  const val = (args || '').trim().toLowerCase();
  if (!val) {
    const on = getConsentForUser(tags) ? 'yes' : 'no';
    return sayWithConsent(channel, tags, `${formatAddress(tags)} touch consent is currently **${on}**. Use "!tc yes" to allow respectful contact gestures or "!tc no" to disable. ${getSignatureForMood(currentMood)}`);
  }
  if (val === 'yes' || val === 'y' ) {
    TOUCH_CONSENT.users[uid] = true;
    saveTouchConsent();
    return sayWithConsent(channel, tags, `${formatAddress(tags)} noted. I will allow respectful contact gestures with your consent. ${getSignatureForMood(currentMood)}`);
  }
  if (val === 'no' || val === 'n') {
    TOUCH_CONSENT.users[uid] = false;
    saveTouchConsent();
    return sayWithConsent(channel, tags, `${formatAddress(tags)} understood. I will not initiate contact and will sanitize contact gestures. ${getSignatureForMood(currentMood)}`);
  }
  return sayWithConsent(channel, tags, `${formatAddress(tags)} usage: !tc yes | !tc no | !tc (to view). ${getSignatureForMood(currentMood)}`);
}
...
// --- Touch consent command: !tc yes|no|<empty>
  if (/^!tc\b/i.test(text)) {
    const args = text.replace(/^!tc\b/i, '').trim();
	await sayWithConsent(channel, tags, `${addressed} ${body} ${sig}`, 480);
    return void handleTouchConsentCommand(channel, tags, args);
  }
...
// --- New: per-user timezone commands ---
  ...
  if (text === '!rules') {
    ...
    await sayWithConsent(channel, tags, twitchSafe(`${addressed} Please be kind, avoid spoilers, and respect the mods. More: !faq • ${cta}`, 480));
    return;
  }
  if (text === '!schedule') {
    dbg('branch=command', 'cmd=!schedule');
	const hello = pickOne(CHARITY_CFG?.style?.lexicon?.greeting_variants, 'Hey');
    await sayWithConsent(channel, tags, twitchSafe(`${addressed} ${hello}! Tue & Thu 8–11 PM, Sat 2–5 PM (ET). Check panel for updates.`, 480));
    return;
  }
  if (text === '!reloadkb' && (tags.mod || tags.badges?.broadcaster === '1')) {
    loadKB();
	dbg('branch=command', 'cmd=!reloadkb');
    await sayWithConsent(channel, tags, `KB reloaded.`);
    return;
  }
  if (text === '!version') {
     dbg('branch=command', 'cmd=!version');
	await sayWithConsent(channel, tags, `Charity v${PKG.version} • ${BOT_USERNAME} on #${CHANNEL}`);
    return;
}
  if (text === '!streamstatus') {
    ...
    if (t) parts.push(t);
    if (g) parts.push(g);
    await sayWithConsent(channel, tags, `${addressed} ${parts.join(' • ')}`);
    return;
}
...
dbg('branch=qa', 'mood=', currentMood, 'question=', JSON.stringify(question || text));
  ...
  try {
    ...
    await sayWithConsent(channel, tags, twitchSafe(`${addressed} ${body} ${sig}`, 480));
