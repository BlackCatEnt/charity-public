...
// --- Consent / Trusted Contact helpers ---
...
function isTrustedForContact(tags) {
  ...
  // Broadcaster always trusted
  return isBroadcasterUser?.(tags) === true || TRUSTED_CONTACTS.has(name) || getConsentForUser(tags) === true;
}
...
function getRoleFromTags(tags, followerKnown = false) {
  const b = tags.badges || {};
  // Primary: badge
  if (b.broadcaster === '1') return 'broadcaster';
...
function formatAddress(tags) {
  ...
  const role = tags ? tags._resolvedRole : 'non_follower'; // allow caller to set
  const roleName = (lex.address_by_role || {})[role] || '';
...
client.on('message', async (channel, tags, message, self) => {
  ...
  const addressed = formatAddress(tags);     // e.g., "Adventurer @Felix" or "Traveler @Felix"
