...
await preflightRefresh(30 * 60, logger); // refresh if <30m left
...
// tmi client — use resolved constants for safety
  const client = new tmi.Client({
	connection: { reconnect: true, secure: true },
	identity: { username: BOT_USERNAME, password: PASSWORD }, // PASSWORD from TWITCH_OAUTH
	channels: [ `#${CHANNEL}` ],
  });
...
client.on('message', async (channel, tags, message, self) => {
  ...
  // Fast, deterministic catch
  if (/^\s*(are you there|you there\??|here\??)\s*$/i.test(text)) {
  ...
  // 2) mention alias → !ask (only if they addressed Charity)
  const replyingToMe =
  ...
  const nickPart = [...nicknames, 'Charity', 'Charity the Adventurer']
    .map(n => (String(n).trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).replace(/\s+/g, '[ _]*'))
    .filter(Boolean)
    .join('|');
  ...
  // 3) classification fast path (small-talk intent etc.)
  const cls = await classify(text);
  const action = await decide({ platform, channel, tags, text, classification: cls });
  if (action?.kind === 'SAY' || action?.kind === 'ASK') {
...
try {
  ...
  const watchdog = createWatchdog(client);
  watchdog.start();
} catch (err) {
