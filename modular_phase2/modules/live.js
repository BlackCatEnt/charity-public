...
// Back-compat wrapper expected by some handlers
  function getContext() {
    return {
      isLive: state.isLive,
      title: state.title,
  ...
  async function helix(pathAndQuery) {
    const headers = await helixHeaders('broadcaster');
    const res = await fetch('https://api.twitch.tv/helix' + pathAndQuery, { headers });
  ...
  async function resolveUserId() {
    ...
    const j = await helix(`/users?login=${encodeURIComponent(login)}`);
  ...
  async function fetchGameName(gameId) {
    ...
    const j = await helix(`/games?id=${encodeURIComponent(gameId)}`);
  ...
  async function fetchOnce() {
    try {
      ...
      const j = await helix(`/streams?user_id=${encodeURIComponent(broadcasterId)}`);
      const live = j?.data?.[0];
