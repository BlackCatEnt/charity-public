...
function toBlob(float32) {
  let vec;
  if (ArrayBuffer.isView(float32) && float32.BYTES_PER_ELEMENT === 4) {
    // Already a Float32Array or another typed view
    vec = new Float32Array(float32.buffer, float32.byteOffset, float32.byteLength / 4);
  } else if (Array.isArray(float32)) {
    vec = Float32Array.from(float32);
  } else if (typeof float32 === 'number') {
    // Fallback: single number → 1-dim vector
    vec = Float32Array.from([float32]);
  } else {
